...
begin if WSAStartup($0202, WSA) <> 0 then Exit;
  try FListenSocket := socket(AF_INET, SOCK_STREAM, IPPROTO_TCP); if FListenSocket = INVALID_SOCKET then Exit;
    ...
    if bind(FListenSocket, Addr, SizeOf(Addr)) <> 0 then begin
      FOwner.DoLog('[����] [�ն˻ص�] �ص���������ʧ�ܣ�bind ' + FOwner.FTerminalCallbackListenHost + ':' +
        IntToStr(FOwner.FTerminalCallbackPort) + ' ʧ�ܣ�error=' + IntToStr(WSAGetLastError) + '��');
      Exit;
    end;
    if listen(FListenSocket, SOMAXCONN) <> 0 then begin
      FOwner.DoLog('[����] [�ն˻ص�] �ص���������ʧ�ܣ�listen ' + FOwner.FTerminalCallbackListenHost + ':' +
        IntToStr(FOwner.FTerminalCallbackPort) + ' ʧ�ܣ�error=' + IntToStr(WSAGetLastError) + '��');
      Exit;
    end;
    FOwner.DoLog('[��Ϣ] [�ն˻ص�] �ص����շ�����������listen=' + FOwner.FTerminalCallbackListenHost + ':' +
      IntToStr(FOwner.FTerminalCallbackPort));
    FOwner.DoLog('[��Ϣ] [�ն˻ص�] �ն˻ص���ַ��' + FOwner.GetCallbackBase);
    while not Terminated do begin
      ...
      try Raw := '';
        ...
        if HeaderEnd > 0 then begin
          ...
          FOwner.DoLog('[��Ϣ] [�ն˻ص�] ���յ��ն˻ص���body_size=' + IntToStr(Length(BodyUtf8)));
...
begin if WSAStartup($0202, WSA) <> 0 then Exit;
  try FListenSocket := socket(AF_INET, SOCK_STREAM, IPPROTO_TCP); if FListenSocket = INVALID_SOCKET then Exit;
    ...
    if bind(FListenSocket, Addr, SizeOf(Addr)) <> 0 then begin
      FOwner.DoLog('[����] [����] DLLͨ�ŷ������ʧ�ܣ�bind ' + FOwner.FDelphiServerHost + ':' +
        IntToStr(FOwner.FDelphiServerPort) + ' ʧ�ܣ�error=' + IntToStr(WSAGetLastError) + '��');
      Exit;
    end;
    if listen(FListenSocket, SOMAXCONN) <> 0 then begin
      FOwner.DoLog('[����] [����] DLLͨ�ŷ������ʧ�ܣ�listen ' + FOwner.FDelphiServerHost + ':' +
        IntToStr(FOwner.FDelphiServerPort) + ' ʧ�ܣ�error=' + IntToStr(WSAGetLastError) + '��');
      Exit;
    end;
    FOwner.DoLog('[��Ϣ] [����] DLLͨ�ŷ�����������http://' + FOwner.FDelphiServerHost + ':' +
    ...
    while not Terminated do begin Client := accept(FListenSocket, nil, nil); if Client = INVALID_SOCKET then Continue;
      try Raw := '';
        ...
        if HeaderEnd > 0 then begin Header := Copy(Raw, 1, HeaderEnd - 1); ContentLength := 0;
          ...
          BodyUtf8 := Copy(Raw, HeaderEnd + 4, ContentLength);
          FOwner.DoLog('[��Ϣ] [DLL����] ���յ�DLL�·�����' + Method + ' ' + Path);
...
begin if FThread <> nil then Exit;
  ...
  DoLog('[��Ϣ] [����] �Ѽ�Ȿ����������ַ��' + FLanIp);
  DoLog('[��Ϣ] [�ն�״̬] ��ǰ�նˣ�' + FTerminalManager.CurrentName + ' ' + FTerminalManager.CurrentBaseUrl);
  FCallbackThread := TCallbackReceiverThread.Create(Self); FCallbackThread.Resume;
  DoLog('[��Ϣ] [����] ��������DLLͨ�ŷ���http://' + FDelphiServerHost + ':' + IntToStr(FDelphiServerPort));
  FThread := TDelphiHttpServerThread.Create(Self); FThread.Resume;
  DoLog('[��Ϣ] [����] DLLͨ���ն˻ص�=' + GetCallbackBase);
  DoLog('[��Ϣ] [Ԥ������] �������������ȴ��ⲿԤ����������'); end;
...
begin if FThread <> nil then begin FThread.StopServer; FThread.WaitFor; FThread.Free; FThread := nil; end;
  ...
  FLocalActivePreviews := []; FExternalActivePreviews := []; DoLog('[��Ϣ] [����] ������ֹͣ��'); end;
...
begin Result := False; if DllCallbackUrl = '' then Exit;
  DoLog('[��Ϣ] [DLL�ص�] ������DLL�ش����������url=' + DllCallbackUrl +
...
begin
  TotalTick := GetTickCount;
  DoLog(Format('[��Ϣ] [�ն��л�] �����ն�%d�ϻָ��Ԥ����', [FTerminalManager.CurrentIndex]));
  for SessionType := pstLocal to pstExternal do
  begin
    if SessionType = pstLocal then
    begin
...
begin
  ...
  if FTerminalManager.IsSameTerminal(Index) then begin
    DoLog('[��ʾ] [�ն��л�] ��ǰ�����ն�' + IntToStr(Index) + '����ˢ������ͷ��ָ��Ԥ����');
  ...
  DoLog('[��Ϣ] [�ն��л�] �����л����ն�' + IntToStr(FTerminalManager.CurrentIndex) + ' -> �ն�' + IntToStr(Index));
  PhaseTick := GetTickCount;
  AutoStopPreviews;
  ...
  DoLog('[��Ϣ] [�ն��л�] ��ǰ�ն����л�Ϊ��' + FTerminalManager.CurrentName + ' ' + FTerminalManager.CurrentBaseUrl);
  PhaseTick := GetTickCount;
  AutoStartPreviews;
...
begin
  ...
  DoLog('[��Ϣ] [����] �������ն˿�ʼ���̣�url=' + BaseUrl + '/process/start��save_dir=' + ResolvedSaveDir);
  DoLog('[��Ϣ] [����] �ն˻ص���ַ=' + GetCallbackBase);
  ...
  DoLog('[��Ϣ] [����] �ն������ѿ�ʼ��save_dir=' + FTerminalManager.ProcessSaveDir);
  Result := True;
end;
...
begin FTerminalManager.ProcessActive := False; FTerminalManager.ProcessSaveDir := '';
  FRequestSaveDirs.Clear; FRequestCallbacks.Clear; DoLog('[��Ϣ] [����] �����ѽ�����'); Result := True; end;
...
begin Result := False; SavePath := ''; BaseUrl := FTerminalManager.CurrentBaseUrl; ReqId := GenRequestId('FACE');
  DoLog('[��Ϣ] [�ն˵���] ������������ץ�ģ�request_id=' + ReqId + '��url=' + BaseUrl + '/resources/face-image/sync-request');
  if not FTerminalClient.PostJson(BaseUrl, '/resources/face-image/sync-request',
      '{"request_id":"' + ReqId + '"}', ResponseUtf8) then begin
    DoLog('[����] [�ն˵���] ����ץ������ʧ�ܣ�terminal=' + BaseUrl); Exit; end;
  DoLog('[��Ϣ] [����ץ��] ���յ��ն���Ӧ��response_size=' + IntToStr(Length(ResponseUtf8)));
  FaceResult := FCallbackParser.ParseImageCapture(ResponseUtf8);
  if not FaceResult.Valid then begin DoLog('[����] [����ץ��] ��Ӧ����ʧ�ܡ�'); Exit; end;
  ...
  if SavePath = '' then begin DoLog('[����] [����ץ��] ͼƬ����ʧ�ܡ�'); Exit; end;
  DoLog('[��Ϣ] [����ץ��] ͼƬ����ɹ���' + SavePath); Result := True; end;
...
begin Result := False; SavePath := ''; BaseUrl := FTerminalManager.CurrentBaseUrl; ReqId := GenRequestId('FP');
  DoLog('[��Ϣ] [�ն˵���] ��������ָ��ץ�ģ�request_id=' + ReqId + '��url=' + BaseUrl + '/resources/fingerprint/sync-request');
  if not FTerminalClient.PostJson(BaseUrl, '/resources/fingerprint/sync-request',
      '{"request_id":"' + ReqId + '"}', ResponseUtf8) then begin
    DoLog('[����] [�ն˵���] ָ��ץ������ʧ�ܣ�terminal=' + BaseUrl); Exit; end;
...
begin Result := ''; BaseUrl := FTerminalManager.CurrentBaseUrl; Result := GenRequestId('OCR');
  ...
  DoLog('[��Ϣ] [�ն˵���] �����ύOCRʶ������request_id=' + Result + '��callback=' + CallbackUrl);
  if not FTerminalClient.PostJson(BaseUrl, '/resources/ocr-document/request',
    ...
    DoLog('[����] [�ն˵���] OCRʶ�������͵��ն�ʧ�ܡ�'); Result := ''; Exit; end;
  FRequestSaveDirs.Values[Result] := SafeResolveSaveDir(SaveDir);
  DoLog('[��Ϣ] [OCRʶ��] �������������ȴ��ն˻ص���' + CallbackUrl); end;
...
begin Result := ''; BaseUrl := FTerminalManager.CurrentBaseUrl; Result := GenRequestId('NFC');
  CallbackUrl := GetCallbackBase;
  DoLog('[��Ϣ] [�ն˵���] �����ύIC��ʶ������request_id=' + Result + '��callback=' + CallbackUrl);
  if not FTerminalClient.PostJson(BaseUrl, '/resources/nfc-card/request',
      '{"request_id":"' + Result + '","callback_url":"' + CallbackUrl + '"}', ResponseUtf8) then begin
    DoLog('[����] [�ն˵���] IC��ʶ�������͵��ն�ʧ�ܡ�'); Result := ''; Exit; end;
  FRequestSaveDirs.Values[Result] := SafeResolveSaveDir(SaveDir);
  DoLog('[��Ϣ] [IC��ʶ��] �������������ȴ�ˢ���ص���'); end;
...
begin Result := ''; BaseUrl := FTerminalManager.CurrentBaseUrl; Result := GenRequestId('IRIS');
  CallbackUrl := GetCallbackBase;
  DoLog('[��Ϣ] [�ն˵���] �����ύ��Ĥץ������request_id=' + Result + '��callback=' + CallbackUrl);
  if not FTerminalClient.PostJson(BaseUrl, '/resources/iris/request',
      '{"request_id":"' + Result + '","callback_url":"' + CallbackUrl + '"}', ResponseUtf8) then begin
    DoLog('[����] [�ն˵���] ��Ĥץ�������͵��ն�ʧ�ܡ�'); Result := ''; Exit; end;
  FRequestSaveDirs.Values[Result] := SafeResolveSaveDir(SaveDir);
  DoLog('[��Ϣ] [��Ĥץ��] �������������ȴ��ն˻ص���'); end;
...
begin
  ...
  if Path = '/capture/iris' then begin
    ...
    try
      ...
      begin DoLog('[��Ϣ] [DLL����] DLL�·��ĺ�Ĥץ��������ת���նˣ�request_id=' + RequestId); Result := '{"accepted":true}'; end
      else begin DoLog('[����] [�ն˵���] DLL�·��ĺ�Ĥץ������ת���ն�ʧ�ܡ�'); Result := '{"error":true,"code":"terminal_request_failed"}'; end;
  ...
  if Path = '/ocr' then begin
    ...
    try
      if Client.PostJson(TerminalBaseUrl, '/resources/ocr-document/request',
          '{"request_id":"' + RequestId + '","callback_url":"' + GetCallbackBase + '"}', ResponseUtf8) then
      begin DoLog('[��Ϣ] [DLL����] DLL�·���OCRʶ��������ת���նˣ�request_id=' + RequestId); Result := '{"accepted":true}'; end
      else begin DoLog('[����] [�ն˵���] DLL�·���OCRʶ������ת���ն�ʧ�ܡ�'); Result := '{"error":true,"code":"terminal_request_failed"}'; end;
    finally Client.Free; end; Exit; end;
  ...
  if Path = '/nfc' then begin
    ...
    try
      if Client.PostJson(TerminalBaseUrl, '/resources/nfc-card/request',
          '{"request_id":"' + RequestId + '","callback_url":"' + GetCallbackBase + '"}', ResponseUtf8) then
      begin DoLog('[��Ϣ] [DLL����] DLL�·���IC��ʶ��������ת���նˣ�request_id=' + RequestId); Result := '{"accepted":true}'; end
      else begin DoLog('[����] [�ն˵���] DLL�·���IC��ʶ������ת���ն�ʧ�ܡ�'); Result := '{"error":true,"code":"terminal_request_failed"}'; end;
    finally Client.Free; end; Exit; end;
  ...
  if Path = '/preview/camera/start' then begin
    ...
    if (ThirdPartyHwndVal = 0) or not IsWindow(ThirdPartyHwndVal) then begin
      DoLog('[����] [Ԥ������] ����ͷĿ�괰�ھ����Ч��hwnd=' + IntToStr(ThirdPartyHwndVal));
      Result := '{"error":true,"code":"invalid_target_hwnd"}'; Exit; end;
    FThirdPartyCameraHwnd := ThirdPartyHwndVal;
    DoLog('[��Ϣ] [DLL����] DLL�·�����ͷԤ������target_hwnd=' + IntToStr(ThirdPartyHwndVal));
  ...
  if Path = '/preview/fingerprint/start' then begin
    ...
    if (FpHwnd = 0) or not IsWindow(FpHwnd) then begin
      DoLog('[����] [Ԥ������] ָ����Ŀ�괰�ھ����Ч��hwnd=' + IntToStr(FpHwnd));
      Result := '{"error":true,"code":"invalid_target_hwnd"}'; Exit; end;
  ...
  if Path = '/preview/iris/start' then begin
    ...
    if (IrisHwnd = 0) or not IsWindow(IrisHwnd) then begin
      DoLog('[����] [Ԥ������] ��ĤĿ�괰�ھ����Ч��hwnd=' + IntToStr(IrisHwnd));
      Result := '{"error":true,"code":"invalid_target_hwnd"}'; Exit; end;
...
begin
  ...
  DoLog('[��Ϣ] [�ն˻ص�] ���ڴ����ն˻ص���resource_type=' + ResourceType + '��request_id=' + RequestId);
  ...
  if ResourceType = 'ocr_event_status' then begin
    ...
    if OcrResult.Valid then begin
      ...
      while I < OcrResult.EvidenceImagesCount do begin
        if OcrResult.EvidenceImages[I].ImageBase64 <> '' then begin
          ...
          DoLog('[��Ϣ] [OCR�ص�] ֤��ͼƬ�ѱ��棺type=' + IntToStr(OcrResult.EvidenceImages[I].ImageType) +
            '��lamp=' + IntToStr(OcrResult.EvidenceImages[I].LampType) + '��path=' + ImgPath);
        end;
        I := I + 1;
      end;
      DoLog('[��Ϣ] [OCR�ص�] ʶ����ɣ�mrz=' + OcrResult.Mrz + '��evidence_count=' + IntToStr(OcrResult.EvidenceImagesCount) + '��save_path=' + SavePath);
    ...
    if NfcResult.Valid then begin
      ...
      if DllCallbackUrl <> '' then begin
        ...
        MakeCallback(RequestId, DllCallbackUrl, PayloadUtf8); end; end
    else DoLog('[����] [IC���ص�] �ص�����ʧ�ܣ�δ�ҵ�card_text��'); end
    ...
    if ImgResult.Valid then begin
      ...
      DoLog('[��Ϣ] [�����ص�] ͼƬ����ɹ���save_path=' + SavePath); end; end
    ...
    if ImgResult.Valid then begin
      ...
      DoLog('[��Ϣ] [ָ�ƻص�] ͼƬ����ɹ���save_path=' + SavePath); end; end

  else DoLog('[��ʾ] [�ն˻ص�] �յ�δ֪��Դ���ͣ�resource_type=' + ResourceType);
