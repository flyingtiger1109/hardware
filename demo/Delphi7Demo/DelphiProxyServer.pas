    ...
    while not Terminated do begin
      ...
      try Raw := '';
        ...
        if HeaderEnd > 0 then begin
          ...
          FOwner.DoLog('[��?] [?????] ??��???����body_size=' + IntToStr(Length(BodyUtf8)));
...
begin
  ...
  try
    ...
    if bind(FListenSocket, Addr, SizeOf(Addr)) <> 0 then begin
      FOwner.DoLog('[����] [����] DLL???������?????bind ' + FOwner.FDelphiServerHost + ':' +
        IntToStr(FOwner.FDelphiServerPort) + ' ?��error=' + IntToStr(WSAGetLastError) + '?��');
      Exit;
    end;
    if listen(FListenSocket, SOMAXCONN) <> 0 then begin
      FOwner.DoLog('[����] [����] DLL???������?????listen ' + FOwner.FDelphiServerHost + ':' +
        IntToStr(FOwner.FDelphiServerPort) + ' ?��error=' + IntToStr(WSAGetLastError) + '?��');
      Exit;
    end;
    FOwner.DoLog('[��?] [����] DLL???��������������http://' + FOwner.FDelphiServerHost + ':' +
...
begin
  ...
  DoLog('[��?] [DLL����] HTTP??������?����������?��?=' + IntToStr(High(FHttpWorkers) - Low(FHttpWorkers) + 1) +
    '����������=' + IntToStr(HTTP_QUEUE_LIMIT));
end;
    ...
    while FQueue.Count >= FMaxQueue do begin
      ...
      if OldTask <> nil then begin
        ...
        try
          ...
          FOwner.DoLog('[����] [?����?] ��������?�I??����?����?��?=' + FWorkerName + '��old_path=' + OldTask.Path + '��new_path=' + Task.Path);
  ...
  while not Terminated do begin
    ...
    try
      if Task.Generation <> FOwner.CurrentSwitchGeneration then begin
        ...
        FOwner.DoLog('[����] [?����?] ������??����?��?=' + FWorkerName + '��path=' + Task.Path);
      end else begin
      ...
      on E: Exception do begin
        ...
        FOwner.DoLog('[����] [?����?] ����??��?������=' + FWorkerName + '������=' + E.Message);
      end;
    end;
    FreeTask := False;
    NotifyTask := False;
...
begin
  ...
  begin
    ...
    FOwner.DoLog('[����] [OCR�ص�] ʶ��������ʧ�ܡ�' + ResourceType +
  ...
  begin
    if not FOwner.IsSwitchGenerationCurrent(FSwitchGeneration) then Exit;
    FOwner.DoLog('[����] [???��] ?��: resource=' + ResourceType +
      ', request_id=' + FRequestId);
...
begin
  ...
  try
    Vlc.Warmup;
    FOwner.DoLog('[��?] [VLC?��] ����?, ��?=' + IntToStr(Vlc.WarmupMs) + 'ms');
  finally
    Vlc.Free;
  end;
end;
...
begin if FThread <> nil then Exit;
  ...
  DoLog('[��?] [����] ????����������??��' + FLanIp);
  DoLog('[��?] [?????] ��????��' + FTerminalManager.CurrentName + ' ' + FTerminalManager.CurrentBaseUrl);
  FCallbackThread := TCallbackReceiverThread.Create(Self); FCallbackThread.Resume;
  StartHttpWorkers;
  StartBusinessWorkers;
  DoLog('[��?] [����] ��������DLL???��?http://' + FDelphiServerHost + ':' + IntToStr(FDelphiServerPort));
  FThread := TDelphiHttpServerThread.Create(Self); FThread.Resume;
  DoLog('[��?] [����] DLL?��?????=' + GetCallbackBase);
  DoLog('[��?] [?������] ����������������?��??��?����...'); TVlcWarmupThread.Create(Self); end;
...
begin if FThread <> nil then begin FThread.StopServer; FThread.WaitFor; FThread.Free; FThread := nil; end;
  ...
  FLocalActivePreviews := []; FExternalActivePreviews := []; DoLog('[��?] [����] ��������????��'); end;
...
begin
  ...
  DoLog('[��?] [DLL???] ������DLL??��?������url=' + DllCallbackUrl +
    '?��body_size=' + IntToStr(Length(PayloadUtf8)));
  Result := HttpPostJson(DllCallbackUrl, PayloadUtf8);
  if Result then
    DoLog('[��?] [DLL???] DLL??��?����request_id=' + RequestId)
  else
    DoLog('[����] [DLL???] DLL??????��request_id=' + RequestId + '?��url=' + DllCallbackUrl);
end;
  ...
  begin
    ...
    for ResType := High(TPreviewResourceType) downto Low(TPreviewResourceType) do
      ...
      begin
        ...
        DoLog(Format('[����] ???�� resource=%d session=%d ��?=%d��?��',
          [Ord(ResType), Ord(SessionType), Integer(GetTickCount - ItemTick)]));
      end;
  end;
  DoLog(Format('[����] ???��?�� ��?=%d��?��', [Integer(GetTickCount - TotalTick)]));
end;
...
begin
  TotalTick := GetTickCount;
  DoLog(Format('[��?] [??����?] ����???%d???��???��', [FTerminalManager.CurrentIndex]));
  for SessionType := pstLocal to pstExternal do
  begin
    if SessionType = pstLocal then
    begin
...
begin
  ...
  try
    ...
    Result := True;
    DoLog(Format('[��?] [??����?] ��������??�ݡ�����terminal=%d��generation=%d', [Index, SwitchGeneration]));
  except
    on E: Exception do
    begin
      EndTerminalSwitch(SwitchGeneration);
...
begin
  ...
  DoLog('[��?] [����] ������?????��??��url=' + BaseUrl + '/process/start??��save_dir=' + ResolvedSaveDir);
  DoLog('[��?] [����] ????��??=' + GetCallbackBase);
  ...
  DoLog('[��?] [����] ??������????��save_dir=' + FTerminalManager.ProcessSaveDir);
  Result := True;
end;
...
begin FTerminalManager.ProcessActive := False; FTerminalManager.ProcessSaveDir := '';
  ClearRequestContexts; DoLog('[��?] [����] ����??������'); Result := True; end;
...
begin Result := False; SavePath := ''; BaseUrl := FTerminalManager.CurrentBaseUrl; ReqId := GenRequestId('FACE');
  DoLog('[��?] [????��] ������??��������????��request_id=' + ReqId + '?��url=' + BaseUrl + '/resources/face-image/sync-request');
  if not FTerminalClient.PostJson(BaseUrl, '/resources/face-image/sync-request',
      '{"request_id":"' + ReqId + '"}', ResponseUtf8) then begin
    DoLog('[����] [????��] ����?��???��???��terminal=' + BaseUrl); Exit; end;
  DoLog('[��?] [����?��] ??��?��???��response_size=' + IntToStr(Length(ResponseUtf8)));
  FaceResult := FCallbackParser.ParseImageCapture(ResponseUtf8);
  if not FaceResult.Valid then begin DoLog('[����] [����?��] ��?����???��'); Exit; end;
  ...
  if SavePath = '' then begin DoLog('[����] [����?��] ??����???��'); Exit; end;
  DoLog('[��?] [����?��] ??����?����' + SavePath); Result := True; end;
...
begin Result := False; SavePath := ''; BaseUrl := FTerminalManager.CurrentBaseUrl; ReqId := GenRequestId('FP');
  DoLog('[��Ϣ] [�ն�ͨ��] �������ն�����ָ��ץ�ģ�request_id=' + ReqId + '��url=' + BaseUrl + '/resources/fingerprint/sync-request');
  if not FTerminalClient.PostJson(BaseUrl, '/resources/fingerprint/sync-request',
      '{"request_id":"' + ReqId + '"}', ResponseUtf8) then begin
    DoLog('[����] [�ն�ͨ��] ָ��ץ��ָ���ʧ�ܣ�terminal=' + BaseUrl); Exit; end;
  FpResult := FCallbackParser.ParseImageCapture(ResponseUtf8);
  if not FpResult.Valid then begin DoLog('[����] [ָ��ץ��] ��Ӧ����ʧ�ܡ�'); Exit; end;
  ...
  if SavePath = '' then begin DoLog('[����] [ָ��ץ��] ͼƬ����ʧ�ܡ�'); Exit; end;
  DoLog('[��Ϣ] [ָ��ץ��] ͼƬ����ɹ���' + SavePath); Result := True; end;
...
begin Result := ''; BaseUrl := FTerminalManager.CurrentBaseUrl; Result := GenRequestId('OCR');
  ...
  DoLog('[��?] [????��] ����??OCR??��request_id=' + Result + '?��callback=' + CallbackUrl);
  if not FTerminalClient.PostJson(BaseUrl, '/resources/ocr-document/request',
    ...
    DoLog('[����] [????��] OCR?��?????��?????��'); Result := ''; Exit; end;
  RememberRequestContext(Result, SaveDir, '');
  DoLog('[��?] [OCR?��] ��???��????...' + CallbackUrl); end;
...
begin Result := ''; BaseUrl := FTerminalManager.CurrentBaseUrl; Result := GenRequestId('NFC');
  CallbackUrl := GetCallbackBase;
  DoLog('[��?] [????��] ����??IC��??��request_id=' + Result + '?��callback=' + CallbackUrl);
  if not FTerminalClient.PostJson(BaseUrl, '/resources/nfc-card/request',
      '{"request_id":"' + Result + '","callback_url":"' + CallbackUrl + '"}', ResponseUtf8) then begin
    DoLog('[����] [????��] IC��?��?????��?????��'); Result := ''; Exit; end;
  RememberRequestContext(Result, SaveDir, '');
  DoLog('[��?] [IC��?��] ��?????��???...'); end;
...
begin Result := ''; BaseUrl := FTerminalManager.CurrentBaseUrl; Result := GenRequestId('IRIS');
  CallbackUrl := GetCallbackBase;
  DoLog('[��?] [????��] ����??��????��request_id=' + Result + '??��callback=' + CallbackUrl);
  if not FTerminalClient.PostJson(BaseUrl, '/resources/iris/request',
      '{"request_id":"' + Result + '","callback_url":"' + CallbackUrl + '"}', ResponseUtf8) then begin
    DoLog('[����] [????��] ��??��?????��?????��'); Result := ''; Exit; end;
  RememberRequestContext(Result, SaveDir, '');
  DoLog('[��?] [��??��] ��???��????...'); end;
...
begin
  if FFaceCaptureWorker = nil then FFaceCaptureWorker := TProxyTaskWorker.Create(Self, '����??��', 1);
  if FFingerprintCaptureWorker = nil then FFingerprintCaptureWorker := TProxyTaskWorker.Create(Self, '?��??��', 1);
  if FOcrWorker = nil then FOcrWorker := TProxyTaskWorker.Create(Self, 'OCR��?', 1);
  if FNfcWorker = nil then FNfcWorker := TProxyTaskWorker.Create(Self, 'IC����?', 1);
  FFaceCaptureWorker.Resume;
  FFingerprintCaptureWorker.Resume;
  FOcrWorker.Resume;
  FNfcWorker.Resume;
  DoLog('[��?] [?����?] ????��??������������??��=1��?��?��=1��OCR=1��IC��=1��?����?1��?����+1��??�������������I???������');
end;
  ...
  if Task.DoneEvent.WaitFor(WaitMs) = wrSignaled then begin
    ...
    try
      if Task.DoneEvent.WaitFor(0) = wrSignaled then
        ...
        DoLog('[����] [?����?] ????������?��??��?timeout������=' + WorkerName + '��path=' + Path + '��timeout=' + IntToStr(WaitMs) + 'ms');
...
begin
  ...
  if Path = '/capture/face' then begin
    Result := QueueBusinessRequest(FFaceCaptureWorker, '����??��', Method, Path, BodyUtf8, BUSINESS_WAIT_TIMEOUT_MS);
    Exit;
  end;
  if Path = '/capture/fingerprint' then begin
    Result := QueueBusinessRequest(FFingerprintCaptureWorker, '?��??��', Method, Path, BodyUtf8, BUSINESS_WAIT_TIMEOUT_MS);
    Exit;
  end;
  if Path = '/ocr' then begin
    Result := QueueBusinessRequest(FOcrWorker, 'OCR��?', Method, Path, BodyUtf8, BUSINESS_WAIT_TIMEOUT_MS);
    Exit;
  end;
  if Path = '/nfc' then begin
    Result := QueueBusinessRequest(FNfcWorker, 'IC����?', Method, Path, BodyUtf8, BUSINESS_WAIT_TIMEOUT_MS);
    Exit;
  end;
  Result := HandleRequestDirect(Method, Path, BodyUtf8);
end;
...
begin
  ...
  if Path = '/capture/iris' then begin
    ...
    try
      ...
      begin DoLog('[��?] [DLL����] DLL?����????��?����?����???��request_id=' + RequestId); Result := '{"accepted":true}'; end
      else begin DoLog('[����] [????��] DLL?����????��?��?����??????��'); Result := '{"error":true,"code":"terminal_request_failed"}'; end;
  ...
  if Path = '/ocr' then begin
    ...
    try
      if Client.PostJson(TerminalBaseUrl, '/resources/ocr-document/request',
          '{"request_id":"' + RequestId + '","callback_url":"' + GetCallbackBase + '"}', ResponseUtf8) then
      begin DoLog('[��?] [DLL����] DLL?����?OCR?��?����?����???��request_id=' + RequestId); Result := '{"accepted":true}'; end
      else begin DoLog('[����] [????��] DLL?����?OCR?��?��?����??????��'); Result := '{"error":true,"code":"terminal_request_failed"}'; end;
    finally Client.Free; end; Exit; end;
  ...
  if Path = '/nfc' then begin
    ...
    try
      if Client.PostJson(TerminalBaseUrl, '/resources/nfc-card/request',
          '{"request_id":"' + RequestId + '","callback_url":"' + GetCallbackBase + '"}', ResponseUtf8) then
      begin DoLog('[��?] [DLL����] DLL?����?IC��?��?����?����???��request_id=' + RequestId); Result := '{"accepted":true}'; end
      else begin DoLog('[����] [????��] DLL?����?IC��?��?��?����??????��'); Result := '{"error":true,"code":"terminal_request_failed"}'; end;
    finally Client.Free; end; Exit; end;
...
begin
  ...
  DoLog('[��?] [?????] ??��???����resource_type=' + ResourceType + '??��request_id=' + RequestId);
  ...
  if ResourceType = 'ocr_event_status' then begin
    ...
    if OcrResult.Valid then begin
      ...
      while I < OcrResult.EvidenceImagesCount do begin
        if OcrResult.EvidenceImages[I].ImageBase64 <> '' then begin
          ImgName := '';
          if OcrResult.EvidenceImages[I].ImageType = 2 then ImgName := '��?��'
          ...
          if ImgName <> '' then begin
            ...
            DoLog('[��?] [OCR] ��???��?: type=' + IntToStr(OcrResult.EvidenceImages[I].ImageType) +
              ',lamp=' + IntToStr(OcrResult.EvidenceImages[I].LampType) + ',path=' + ImgPath);
          end;
        end;
        I := I + 1;
      end;
      DoLog('[��?] [OCR] ����?, mrz=' + OcrResult.Mrz + ',evidence_count=' + IntToStr(OcrResult.EvidenceImagesCount) + ',save_path=' + SavePath);
    ...
    if NfcResult.Valid then begin
      DoLog('[��?] [IC��???] ��??����?��card_text=' + Utf8ToAnsi(NfcResult.CardText));
    ...
    if ImgResult.Valid then begin
      ...
      DoLog('[��?] [����???] ??����?����save_path=' + SavePath); end; end
  ...
  else DoLog('[��?] [?????] ???��?��?��??��resource_type=' + ResourceType);

  Result := '{"status":"accepted"}';
  except
    on E: Exception do begin
      DoLog('[����] [?????] ����????��??��resource_type=' + ResourceType +
        '?��request_id=' + RequestId + '������=' + E.Message);
      if DllCallbackUrl = '' then begin
