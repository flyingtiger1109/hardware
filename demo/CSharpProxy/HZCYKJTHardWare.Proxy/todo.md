...
## 2026-06-22 UI 日志虚拟化与当日历史分页

### 本次修改内容

1. 将日志区从无限追加的 `TextBox` 改为 `ListView.VirtualMode`，避免 `AppendText`、`Lines` 全量重建和强制滚动阻塞 UI 线程。
2. UI 订阅 Logger 后台写入事件，实时逐条显示成功、失败和其他文件日志记录；DLL、HTTP、回调接口和现有日志文本格式不变。
3. 实时文本仅保留最近 10000 条；当天完整日志继续写文件。UI 仅从本次 EXE 启动时的文件偏移开始建立索引，启动前已存在的当天日志不加载。
4. 用户向上滚动时，按 500 条一页后台读取本次运行期间更早的日志，到本次启动点停止；跨日后清空视图并切换到新当天日志，不读取前一天文件。
5. 日志列表支持 `Ctrl+C` 与右键“复制选中日志”，可按显示顺序复制一条或多条完整日志。
6. 统一新增可见日志的中文描述；移除窗体层重复的“服务已启动/服务已停止”，VLC 预热完成只保留一条汇总记录。
7. 修复实时日志在文件索引完成后被“正在读取当天历史日志”占位覆盖的问题：最近 10000 条保持内存文本，历史页加载后只刷新可见虚拟行。
8. UI 日志恢复为仅显示业务层 `AppendLog` 记录；队列、VLC/MJPEG、配置与性能等诊断日志只写当天完整文件，避免界面冗余。底部跟随改为按最后一项的实际可见位置判断。
9. 人脸/指纹抓拍统一由服务层按最终结果写一条日志，避免窗体层重复成功记录；失败时不再误报“图片保存成功”。`log.level=debug` 时才写入 `[性能]` 终端切换明细，默认 `info` 不写。
10. OCR/NFC 默认仅保留业务结果：MRZ、读卡结果、OCR结果/MRZ/照片保存目录及异常；事件流、回调映射、重复回调、请求标识和逐文件保存明细仅在 `debug` 级别记录。
11. C# Proxy 与 DLL 均优先读取环境变量 `HZCYKJTHARDWARE_CONFIG` 指定的 `HZCYKJTHardWare.json`，未设置时回退模块同目录；保留 JSON 结构，不切换 INI。
12. UI 日志改用本次运行临时二进制偏移索引：内存只保留最近 10000 条业务日志，历史按 500 条页面从当天日志文件读取，退出时删除临时索引。

### 涉及文件

- `MainForm.cs`：虚拟日志列表刷新、当前底部跟随和历史页预取。
- `MainForm.Designer.cs`：日志控件替换为虚拟 `ListView`。
- `Infrastructure/Logger.cs`：增加后台写入完成事件和当前日期日志路径访问器。
- `Infrastructure/UiLogHistoryStore.cs`：新增当天日志偏移索引、历史页缓存和后台读取组件。

### 兼容性说明

- DLL 导出、调用约定、第三方调用参数：未修改。
- HTTP 请求/响应、回调签名：未修改；仅将新增可见日志的描述文本汉化并去除重复项。
- UI 日志：逐条显示业务层 `AppendLog` 记录；完整诊断信息保留在当天日志文件；历史仅限当日，不跨天。
- 日志级别：`HZCYKJTHardWare.json` 的 `log.level` 为 `info`（默认）时不写性能调试日志；设为 `debug` 并重启 EXE 后启用。
- 配置路径：设置 `HZCYKJTHARDWARE_CONFIG=绝对路径\HZCYKJTHardWare.json` 后，DLL 与 C# Proxy 均优先读取该文件；适用于两者部署在不同目录的场景。

### 验证状态

- [x] x86 Release 编译验证：通过，0 警告、0 错误。
- [ ] 高并发抓拍下 UI 实时显示验证：待验证。
- [ ] 当日历史向上滚动分页验证：待验证。
- [ ] 跨日清空且不读取前一天日志验证：待验证。
- [ ] 24 小时长稳及第三方 UI 响应验证：待验证。
- [ ] 高DPI环境验证（125%/150%缩放）
