...
bool HttpClient::PostJson(const std::string& url,
    ...
    if (!m_hSession) {
        LOG_DEBUG_RATE_LIMITED("HTTP|POST|session", "HTTP请求",
    ...
    if (!WinHttpCrackUrl(wUrl.c_str(), 0, 0, &urlComp)) {
        LOG_DEBUG_RATE_LIMITED("HTTP|POST|url_parse", "HTTP请求",
    ...
    if (!hConnect) {
        LOG_DEBUG_RATE_LIMITED("HTTP|POST|connect", "HTTP请求",
    ...
    if (!hRequest) {
        LOG_DEBUG_RATE_LIMITED("HTTP|POST|open_request", "HTTP请求",
    ...
    if (!WinHttpSendRequest(hRequest, headers, (DWORD)-1,
        ...
        LOG_DEBUG_RATE_LIMITED("HTTP|POST|send", "HTTP请求",
    ...
    if (!WinHttpReceiveResponse(hRequest, nullptr)) {
        LOG_DEBUG_RATE_LIMITED("HTTP|POST|receive", "HTTP请求",
...
bool HttpClient::PostBinary(const std::string& url,
    ...
    if (!m_hSession) {
        LOG_DEBUG_RATE_LIMITED("HTTP|POST_BINARY|session", "HTTP请求",
    ...
    if (!WinHttpCrackUrl(wUrl.c_str(), 0, 0, &urlComp)) {
        LOG_DEBUG_RATE_LIMITED("HTTP|POST_BINARY|url_parse", "HTTP请求",
    ...
    if (!hConnect) {
        LOG_DEBUG_RATE_LIMITED("HTTP|POST_BINARY|connect", "HTTP请求",
    ...
    HINTERNET hRequest = WinHttpOpenRequest(
        ...
        WINHTTP_DEFAULT_ACCEPT_TYPES, flags);
    if (!hRequest) {
        LOG_DEBUG_RATE_LIMITED("HTTP|POST_BINARY|open_request", "HTTP请求",
    ...
    if (!WinHttpSendRequest(hRequest, headers, (DWORD)-1,
        ...
        LOG_DEBUG_RATE_LIMITED("HTTP|POST_BINARY|send", "HTTP请求",
    ...
    if (!WinHttpReceiveResponse(hRequest, nullptr)) {
        const DWORD error = GetLastError();
        if (error == ERROR_WINHTTP_TIMEOUT) responseStatusCode = -2;
        LOG_DEBUG_RATE_LIMITED("HTTP|POST_BINARY|receive", "HTTP请求",
    ...
    while (true) {
        if (!WinHttpQueryDataAvailable(hRequest, &bytesAvailable)) {
            const DWORD error = GetLastError();
            if (error == ERROR_WINHTTP_TIMEOUT) responseStatusCode = -2;
            LOG_DEBUG_RATE_LIMITED("HTTP|POST_BINARY|read_available", "HTTP请求",
        ...
        if (maxResponseBytes > 0 &&
            ...
            LOG_DEBUG_RATE_LIMITED("HTTP|POST_BINARY|response_too_large", "HTTP请求",
        ...
        if (!WinHttpReadData(hRequest, buffer, bytesToRead, &bytesRead)) {
            const DWORD error = GetLastError();
            if (error == ERROR_WINHTTP_TIMEOUT) responseStatusCode = -2;
            LOG_DEBUG_RATE_LIMITED("HTTP|POST_BINARY|read", "HTTP请求",
...
bool HttpClient::Get(const std::string& url,
    ...
    if (!m_hSession) {
        if (!quiet)
            LOG_DEBUG_RATE_LIMITED("HTTP|GET|session", "HTTP请求",
    ...
    if (!WinHttpCrackUrl(wUrl.c_str(), 0, 0, &urlComp)) {
        if (!quiet)
            LOG_DEBUG_RATE_LIMITED("HTTP|GET|url_parse", "HTTP请求",
    ...
    if (!hConnect) {
        if (!quiet)
            LOG_DEBUG_RATE_LIMITED("HTTP|GET|connect", "HTTP请求",
    ...
    if (!hRequest) {
        if (!quiet)
            LOG_DEBUG_RATE_LIMITED("HTTP|GET|open_request", "HTTP请求",
    ...
    if (!WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0,
        ...
        if (!quiet)
            LOG_DEBUG_RATE_LIMITED("HTTP|GET|send", "HTTP请求",
    ...
    if (!WinHttpReceiveResponse(hRequest, nullptr)) {
        if (!quiet)
            LOG_DEBUG_RATE_LIMITED("HTTP|GET|receive", "HTTP请求",
